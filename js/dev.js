...
function animateDecor() {
    ...
    let $moveElement = $('.decor');

    $moveElement.each(function () {
        let $this = $(this);
        let offset = ($this.offset().top / 2) - ($('.header').innerHeight() / 2);
    ...
    $(window).scroll(function () {
        ...
        $moveElement.each(function () {
            ...
            if (direction === "reverse") {
                shiftDistance = st * speed;
                $(this).css("transform", `translateY(${Number(shiftDistance) + Number(offset)}px)`);
            } else {
                shiftDistance = -st * speed;
            }

            if (max && !direction) {
                if (shiftDistance < max) {
                    shiftDistance += ((max * -1) - 10);
                    $(this).css("transform", `translateY(${Number(shiftDistance) + Number(offset)}px)`);
                }
            } else {
                $(this).css("transform", `translateY(${Number(shiftDistance) + Number(offset)}px)`);
    ...
    $window.on('scroll', function () {
        ...
        if (((scrollTop + $window.innerHeight()) > sectionOffset) && (scrollTop < (sectionOffset + section.innerHeight()))) {
            ...
            for (let i = 0; img.length > i; i++) {
                let $this = $(img[i]);
                let itemOffsetTop = $this.offset().top - $('.header').innerHeight();
...
function parallaxFirstAnim(el) {
    let section = $(`.${el}`);
    let activeElOffset = (section.offset().top + $('.header').innerHeight()) + ($window.innerHeight() / 2);
    let scrollBottom = $window.scrollTop() + $window.innerHeight();
    let mbox = section.find('.mbox');

    if (scrollBottom > activeElOffset) {
        section.addClass('on-focus');
    } else {
        section.removeClass('on-focus');
    }
...
function scrollSpeed(el) {
    ...
    $window.on('wheel', function () {
        ...
        if ((scrollTop > section.offset().top) && (scrollTop < (section.offset().top + section.innerHeight() - $window.innerHeight()))) {
            console.log('11111');
            if (oldScrollTop < scrollTop) {
                $window.scrollTop(scrollTop + 25);
            } else {
                $window.scrollTop(scrollTop - 25);
            }
        }
...
$(document).ready(function () {
    ...
    SmoothScroll({stepSize: 50, touchpadSupport: true, animationTime: 1000});
})